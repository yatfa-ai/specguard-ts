...
| 1 | at least one annotation is malformed, or well-formed but unreachable — stacked above another comment-form `@intent:` line (the one-line lookback only ever claims the line directly above the test), or trailing on a `describe`/`suite`/`context` group line, which no lookback can ever claim; the rest of the stack is dead metadata |
